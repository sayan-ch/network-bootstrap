...
approx.trace <- function(A, d = 2, p = 3){
...
if(!dir.exists("approx_trace"))
  dir.create("approx_trace")
...
), "approx_trace/approx_trace_all.csv")
...
write_csv(data.table(
  ...
  runtime = NA
), "approx_trace/approx_trace_coverage.csv")
...
# estimating the expected approx trace
exp.approx.trace <- vector()
...
for(ii in seq_along(nn)){
  ...
  exp.approx.trace[ii] <- mean(as.numeric(out), na.rm = T)
}
  ...
  for(ii in seq_along(nn)){
    ...
    time.ns.boot[ii] <- system.time(
      ns.boot[[ii]] <- make_CI(node.samp.boot(
        A = gen$A, B = 100, eta = nn[ii], fn = approx.trace, ncore = ncnc),
        nn = nn[ii], method = "NSBoot", alpha = 0.05, 
        expected = exp.approx.trace[ii], theta.hat = theta.hat,
        s = 0, o = 0, d = dd)
    )[3]
    ...
    #     nn = nn[ii], method = "ASEWapproxhted", alpha = 0.05,
    #     expected = exp.approx.trace, theta.hat = theta.hat,
    #     s = 0, o = 0, d = dd)
    # )[3]
    ...
    plot.table[[rr]][[ii]] <- bind_rows(
      ...
      ase.par[[ii]]$plot.table,
      # ase.wgt[[ii]]$plot.table, 
      ...
      for(oi in oo){
        ...
        time.son.par[ii] <- system.time(
          son.par[[ii]] <- make_CI(SONNET.boot(
            ...
            boot.prop = 1, fn = approx.trace, ncore = ncnc),
        ...
        #     nn = nn[ii], method = "SONNETWapproxhted", alpha = 0.05,
        #     expected = exp.approx.trace, theta.hat = theta.hat,
        #     s = si, o = oi, d = dd)
        # )[3]
        ...
        time.son.nonpar[ii] <- system.time({
          tmp <- as.numeric(SONNET.boot(A = gen$A, B = 100, s = si, o = oi, d = dd, 
                                        nonpara = T, boot.prop = 1, fn = approx.trace,
                                        ncore = ncnc))
          
          tmp2 <- tmp[!is.na(tmp)]
          
          son.nonpar[[ii]] <- make_CI(tmp2,
    ...
    write_csv(plot.table[[rr]][[ii]], "approx_trace/approx_trace_all.csv", 
              append = T)
    
    write_csv(cover.table[[rr]][[ii]], "approx_trace/approx_trace_coverage.csv",
              append = T)
    
    gc()
  }
  
  save.image(file = "approx_trace/tmp_rdata.RData")
