...
write_csv(data.table(
  ...
  runtime = NA
), "tri_den/tri_density_coverage.csv")
...
exp.tri.density <- (aa*(aa+1)/((aa+bb)*(aa+bb+1)))^3
...
for(rr in 1:enum){
  ...
  for(ii in seq_along(nn)){
    ...
    theta.hat <- tri.den(gen$A)
    
    time.ns.boot[ii] <- system.time(
      ns.boot[[ii]] <- make_CI(node.samp.boot(
        A = gen$A, B = 100, eta = nn[ii], fn = tri.den, ncore = ncnc),
        nn = nn[ii], method = "NSBoot", alpha = 0.05, 
        expected = exp.tri.density, theta.hat = theta.hat,
        s = 0, o = 0, d = dd)
    )[3]
    ...
    time.ase.wgt[ii] <- system.time(
      ase.wgt[[ii]] <- make_CI(ASE.boot.wgt(
        A = gen$A, B = 100, m = 2, h = h.tri.den, d = dd, ncore = ncnc),
    ...
    plot.table[[rr]][[ii]] <- bind_rows(
      ns.boot[[ii]]$plot.table, ase.par[[ii]]$plot.table,
      ase.wgt[[ii]]$plot.table, ase.nonpar[[ii]]$plot.table
    ) %>%
      mutate(rep = rr, .before = 1)
    
    cover.table[[rr]][[ii]] <- bind_rows(
      ns.boot[[ii]]$coverage, ase.par[[ii]]$coverage,
      ase.wgt[[ii]]$coverage, ase.nonpar[[ii]]$coverage
    ) %>%
      mutate(rep = rr, .before = 1) %>%
      mutate(runtime = c(
        time.ns.boot[ii], time.ase.par[ii], time.ase.wgt[ii], time.ase.nonpar[ii]
      ))
      ...
      for(oi in oo){
        ...
        time.son.par[ii] <- system.time(
          son.par[[ii]] <- make_CI(SONNET.boot(
            ...
            boot.prop = 1, fn = tri.den, ncore = ncnc),
        ...
        plot.table[[rr]][[ii]] <- bind_rows(
          plot.table[[rr]][[ii]],
          bind_rows(son.par[[ii]]$plot.table,
        ...
        cover.table[[rr]][[ii]] <- bind_rows(
          cover.table[[rr]][[ii]],
          bind_rows(son.par[[ii]]$coverage,
                    son.wgt[[ii]]$coverage,
                    son.nonpar[[ii]]$coverage) %>%
            mutate(rep = rr, .before = 1) %>%
            mutate(runtime = c(time.son.par[ii],
                               time.son.wgt[ii],
                               time.son.nonpar[ii]))
        )
    ...
    write_csv(plot.table[[rr]][[ii]], "tri_den/tri_density_all.csv", 
              append = T)
    
    write_csv(cover.table[[rr]][[ii]], "tri_den/tri_density_coverage.csv",
              append = T)
  }
  
  save.image(file = "tri_den/tmp_rdata.RData")
