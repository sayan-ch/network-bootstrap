...
write_csv(data.table(
  ...
  runtime = NA
), "edge_den/edge_density_coverage.csv")
...
exp.edge.density <- dd*(aa/(aa+bb))^2
...
for(rr in 1:enum){
  ...
  for(ii in seq_along(nn)){
    ...
    theta.hat <- avg.deg(gen$A)
    
    time.ns.boot[ii] <- system.time(
      ns.boot[[ii]] <- make_CI(node.samp.boot(
        A = gen$A, B = 100, eta = nn[ii], fn = avg.deg, ncore = ncnc),
        nn = nn[ii], method = "NSBoot", alpha = 0.05, 
        expected = exp.edge.density, theta.hat = theta.hat,
        s = 0, o = 0, d = dd)
    )[3]
    ...
    time.ase.wgt[ii] <- system.time(
      ase.wgt[[ii]] <- make_CI(ASE.boot.wgt(
        A = gen$A, B = 100, m = 2, h = h.avg.deg, d = dd, ncore = ncnc),
      ...
      for(oi in oo){
        ...
        time.son.par[ii] <- system.time(
          son.par[[ii]] <- make_CI(SONNET.boot(
            ...
            boot.prop = 1, fn = avg.deg, ncore = ncnc),
    ...
    write_csv(plot.table[[rr]][[ii]], "edge_den/edge_density_all.csv", 
              append = T)
    
    write_csv(cover.table[[rr]][[ii]], "edge_den/edge_density_coverage.csv",
              append = T)
  }
  
  save.image(file = "edge_den/tmp_rdata.RData")
