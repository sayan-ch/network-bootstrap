...
if(!dir.exists("large_eigen"))
  dir.create("large_eigen")
...
), "large_eigen/large_eigen_all.csv")
...
write_csv(data.table(
  ...
  runtime = NA
), "large_eigen/large_eigen_coverage.csv")
...
# estimating the expected largest eigenvalue
exp.large.eigen <- vector()
...
for(ii in seq_along(nn)){
  ...
  exp.large.eigen[ii] <- mean(as.numeric(out), na.rm = T)
}
...
for(rr in 1:enum){
  ...
  for(ii in seq_along(nn)){
    ...
    time.ns.boot[ii] <- system.time({
      tmp <- as.numeric(node.samp.boot(
        A = gen$A, B = 100, eta = nn[ii], fn = largest.eig, ncore = ncnc))
      
      tmp2 <- tmp[!is.na(tmp)]
      
        ns.boot[[ii]] <- make_CI(tmp2,
        nn = nn[ii], method = "NSBoot", alpha = 0.05, 
        expected = exp.large.eigen[ii], theta.hat = theta.hat,
        s = 0, o = 0, d = dd)}
    )[3]
    
    time.ase.par[ii] <- system.time({
      tmp <- as.numeric(ASE.boot(
        A = gen$A, B = 100, eta = nn[ii], d = dd, nonpara = F,
        fn = largest.eig, ncore = ncnc))
    ...
    #     expected = exp.large.eigen, theta.hat = theta.hat,
    #     s = 0, o = 0, d = dd)
    # )[3]
    ...
    for(si in ss)
      for(oi in oo){
        ...
        time.son.par[ii] <- system.time({
          tmp <- as.numeric(SONNET.boot(
            A = gen$A, B = 100, s = si, o = oi, d = dd, nonpara = F,
            boot.prop = 1, fn = largest.eig, ncore = ncnc))
        ...
        time.son.nonpar[ii] <- system.time({
          tmp <- as.numeric(SONNET.boot(A = gen$A, B = 100, s = si, o = oi, d = dd, 
                              nonpara = T, boot.prop = 1, fn = largest.eig,
                             ncore = ncnc))
    ...
    write_csv(plot.table[[rr]][[ii]], "large_eigen/large_eigen_all.csv", 
              append = T)
    
    write_csv(cover.table[[rr]][[ii]], "large_eigen/large_eigen_coverage.csv",
              append = T)
    
    gc()
  }
  
  save.image(file = "large_eigen/tmp_rdata.RData")
