...
if(!dir.exists("eig_ratio"))
  dir.create("eig_ratio")
...
write_csv(data.table(
  ...
  runtime = NA
), "eig_ratio/eig_ratio_coverage.csv")
...
# estimating the expected eig ratio
exp.eig.ratio <- vector()
...
for(ii in seq_along(nn)){
  ...
  exp.eig.ratio[ii] <- mean(as.numeric(out), na.rm = T)
}
  ...
  for(ii in seq_along(nn)){
    ...
    time.ns.boot[ii] <- system.time(
      ns.boot[[ii]] <- make_CI(node.samp.boot(
        A = gen$A, B = 100, eta = nn[ii], fn = eig.ratio, ncore = ncnc),
        nn = nn[ii], method = "NSBoot", alpha = 0.05, 
        expected = exp.eig.ratio[ii], theta.hat = theta.hat,
        s = 0, o = 0, d = dd)
    )[3]
    ...
    #     expected = exp.eig.ratio, theta.hat = theta.hat,
    #     s = 0, o = 0, d = dd)
    # )[3]
      ...
      for(oi in oo){
        ...
        time.son.par[ii] <- system.time(
          son.par[[ii]] <- make_CI(SONNET.boot(
            ...
            boot.prop = 1, fn = eig.ratio, ncore = ncnc),
        ...
        time.son.nonpar[ii] <- system.time({
          tmp <- as.numeric(SONNET.boot(A = gen$A, B = 100, s = si, o = oi, d = dd, 
                                        nonpara = T, boot.prop = 1, fn = eig.ratio,
                                        ncore = ncnc))
    ...
    write_csv(plot.table[[rr]][[ii]], "eig_ratio/eig_ratio_all.csv", 
              append = T)
    
    write_csv(cover.table[[rr]][[ii]], "eig_ratio/eig_ratio_coverage.csv",
              append = T)
    
    gc()
  }
  
  save.image(file = "eig_ratio/tmp_rdata.RData")
