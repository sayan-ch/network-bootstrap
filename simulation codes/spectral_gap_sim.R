...
if(!dir.exists("spectral_gap"))
  dir.create("spectral_gap")
...
), "spectral_gap/spectral_gap_all.csv")
...
write_csv(data.table(
  ...
  runtime = NA
), "spectral_gap/spectral_gap_coverage.csv")
...
# estimating the expected spectral gap
exp.spectral.gap <- vector()
...
for(ii in seq_along(nn)){
  ...
  exp.spectral.gap[ii] <- mean(as.numeric(out), na.rm = T)
}
...
for(rr in 1:enum){
  ...
  for(ii in seq_along(nn)){
    ...
    #     expected = exp.spectral.gap, theta.hat = theta.hat,
    #     s = 0, o = 0, d = dd)
    # )[3]
      ...
      for(oi in oo){
        ...
        time.son.nonpar[ii] <- system.time({
          tmp <- as.numeric(SONNET.boot(A = gen$A, B = 100, s = si, o = oi, d = dd, 
                                        nonpara = T, boot.prop = 1, fn = spectral.gap,
                                        ncore = ncnc))
    ...
    write_csv(plot.table[[rr]][[ii]], "spectral_gap/spectral_gap_all.csv", 
              append = T)
    
    write_csv(cover.table[[rr]][[ii]], "spectral_gap/spectral_gap_coverage.csv",
              append = T)
    
    gc()
  }
  
  save.image(file = "spectral_gap/tmp_rdata.RData")
